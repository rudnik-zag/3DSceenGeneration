...
OUTPUT_DIR   = "/media/dusan/New Volume/ML_DATASET/ARMCHAIRS/007_bergere/007_bergere//output_renders"     # folder for cam_00.jpg ... cam_15.jpg
...
def main():
    ...
    if RENDER_MODE.upper() == "PER_OBJECT" and len(meshes) > 1:
        ...
        for target_mesh in meshes:
            ...
            render_dir = os.path.join(OUTPUT_DIR, safe_dir_name(target_mesh.name))
        ...
        for i, cam in enumerate(cameras):
            scene.camera = cam
            scene.render.filepath = os.path.join(OUTPUT_DIR, f"cam_{i:02d}")
            print("Rendering:", scene.render.filepath + ".jpg")
            bpy.ops.render.render(write_still=True)

        print("DONE ✅")
        print(f"Saved {len(cameras)} renders to: {OUTPUT_DIR}")
