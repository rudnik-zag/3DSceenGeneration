...
# =========================
MODEL_PATH   = "/media/dusan/New Volume/ML_DATASET/ARMCHAIRS/007_bergere/007_bergere/Bergere_2011_Vray2.0.max"     # .max, .fbx, .obj, .glb/.gltf supported
...
OUTPUT_DIR   = "/media/dusan/New Volume/ML_DATASET/ARMCHAIRS/007_bergere/007_bergere"     # base folder; renders go into OUTPUT_DIR/output_renders
...
# Rendering mode
RENDER_MODE = "FULL_SCENE"  # "FULL_SCENE" or "PER_OBJECT"

# Dataset JSON mode
USE_DATASET_JSON = True
DATASET_JSON_PATH = "/media/dusan/New Volume/ML_DATASET/dataset_information.json"
FAILURES_JSON = "render_failures.json"
...
def load_dataset_tasks(json_path):
    ...
    for entries in classes.values():
        for entry in entries:
            fbx_files = entry.get("fbx_files") or []
            max_files = entry.get("max_files") or []
            model_path = fbx_files[0] if fbx_files else (max_files[0] if max_files else None)
            if model_path:
                tasks.append(model_path)
    return tasks

# =========================
# Main
# =========================
def render_one(model_path, output_dir):
    final_output_dir = os.path.join(output_dir, "output_renders")
    ...
    if RENDER_MODE.upper() == "PER_OBJECT" and len(meshes) > 1:
        ...
        for target_mesh in meshes:
            ...
            cam_r = float(CAMERA_RADIUS) if CAMERA_RADIUS is not None else max(cam_r_fit, cam_r_bounds)
            ...
            render_dir = os.path.join(final_output_dir, safe_dir_name(target_mesh.name))
        ...
        print("DONE ✅")
        print(f"Saved renders per-object under: {final_output_dir}")
    else:
        # Compute bounds from visible meshes only
        visible_meshes = get_visible_meshes(imported)
        if not visible_meshes:
            visible_meshes = meshes
        center, radius, min_v, max_v = bounds_center_and_radius(visible_meshes)
        print("Visible bounds center:", center, "radius:", radius)

        # Recenter ALL imported objects to origin (not just meshes)
        if RECENTER_TO_ORIGIN:
            delta = Vector((0, 0, 0)) - center
            translate_objects(imported, delta)
            bpy.context.view_layer.update()
            ...
            if not visible_meshes:
                visible_meshes = meshes
            center, radius, min_v, max_v = bounds_center_and_radius(visible_meshes)
            print("Recentered. New visible center:", center)

        # Create look-at target (aim slightly above the center, like chest level)
        # This avoids many renders looking at the floor.
        height = max(1e-6, (max_v.z - min_v.z))
        aim_point = Vector((center.x, center.y, min_v.z + 0.6 * height))
        look_at = make_empty("LOOK_AT", aim_point)
        ...
        for i, cam in enumerate(cameras):
            scene.camera = cam
            scene.render.filepath = os.path.join(final_output_dir, f"cam_{i:02d}")
            print("Rendering:", scene.render.filepath + ".jpg")
            bpy.ops.render.render(write_still=True)

        print("DONE ✅")
        print(f"Saved {len(cameras)} renders to: {final_output_dir}")

if __name__ == "__main__":
    if USE_DATASET_JSON:
        tasks = load_dataset_tasks(DATASET_JSON_PATH)
        print(f"Found {len(tasks)} tasks in dataset JSON.")
        failures = []
        for model_path in tasks:
            output_dir = os.path.dirname(model_path)
            ensure_dir(output_dir)
            try:
                render_one(model_path, output_dir)
            except Exception as exc:
                print(f"ERROR: failed to import/render {model_path} -> {exc}")
                failures.append({"path": model_path, "error": str(exc)})
                continue
        if failures:
            failures_path = os.path.join(os.path.dirname(DATASET_JSON_PATH), FAILURES_JSON)
